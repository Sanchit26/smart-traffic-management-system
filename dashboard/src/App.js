        ...
        <div className="flex flex-1 pt-16">
          ...
          <div className={`transition-all duration-300 bg-white border-r border-gray-200 ${sidebarOpen ? 'w-64' : 'w-20'} flex flex-col`}>
            <nav className="flex-1 p-4 space-y-2">
              <button className={`w-full text-left px-4 py-2 rounded-lg font-medium ${selectedPanel === 'automation' ? 'bg-blue-100 text-blue-700' : 'text-gray-700 hover:bg-gray-100'}`} onClick={() => setSelectedPanel('automation')}>
                {sidebarOpen ? 'Automation' : '🤖'}
              </button>
              <button className={`w-full text-left px-4 py-2 rounded-lg font-medium ${selectedPanel === 'analytics' ? 'bg-blue-100 text-blue-700' : 'text-gray-700 hover:bg-gray-100'}`} onClick={() => setSelectedPanel('analytics')}>
                {sidebarOpen ? 'Analytics' : '📊'}
              </button>
              <button className={`w-full text-left px-4 py-2 rounded-lg font-medium ${selectedPanel === 'chatbot' ? 'bg-blue-100 text-blue-700' : 'text-gray-700 hover:bg-gray-100'}`} onClick={() => setSelectedPanel('chatbot')}>
                {sidebarOpen ? 'AI Assistant' : '💬'}
              </button>
              <button className={`w-full text-left px-4 py-2 rounded-lg font-medium ${selectedPanel === 'emergency' ? 'bg-blue-100 text-blue-700' : 'text-gray-700 hover:bg-gray-100'}`} onClick={() => setSelectedPanel('emergency')}>
                {sidebarOpen ? 'Emergency' : '🚨'}
              </button>
              <button className={`w-full text-left px-4 py-2 rounded-lg font-medium ${selectedPanel === 'manual' ? 'bg-blue-100 text-blue-700' : 'text-gray-700 hover:bg-gray-100'}`} onClick={() => setSelectedPanel('manual')}>
                {sidebarOpen ? 'Manual Control' : '🎛️'}
              </button>
            </nav>
          </div>
