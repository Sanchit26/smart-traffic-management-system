# Traditional Traffic Signal Simulation
# This version assigns a fixed green time of 60 seconds to each signal, regardless of vehicle count.
...
# Default values for traditional method
TRADITIONAL_GREEN = 30
TRADITIONAL_YELLOW = 5
TRADITIONAL_RED = 180  # enough to cover the other signals' green+yellow
...
# --- Vehicle and simulation setup (copied and simplified from simulation.py) ---
...
class Vehicle(pygame.sprite.Sprite):
    def __init__(self, lane, vehicleClass, direction_number, direction, will_turn):
        ...
        try:
            ...
            fallback = "images/" + direction + "/car.png"
    ...
    def move(self):
        if(self.direction=='right'):
            ...
            if(self.willTurn==1):
                if(self.crossed==0 or self.x+self.currentImage.get_rect().width<700):
            ...
            if(self.crossed==0 and self.y+self.currentImage.get_rect().height>stopLines[self.direction]):
                ...
                vehicles[self.direction]['crossed'] += 1
            if(self.willTurn==1):
                if(self.crossed==0 or self.y+self.currentImage.get_rect().height<450):
            ...
            if(self.crossed==0 and self.x<stopLines[self.direction]):
                ...
                vehicles[self.direction]['crossed'] += 1
            if(self.willTurn==1):
                if(self.crossed==0 or self.x>695):
            ...
            if(self.crossed==0 and self.y<stopLines[self.direction]):
                ...
                vehicles[self.direction]['crossed'] += 1
            if(self.willTurn==1):
                if(self.crossed==0 or self.y>400):
...
# --- Vehicle generation and simulation time ---
def generateVehicles():
    while(True):
        ...
        if(temp<a[0]):
            ...
            direction_number = 3
        v = Vehicle(lane_number, vehicleTypes[vehicle_type], direction_number, directionNumbers[direction_number], will_turn)
        time.sleep(0.75)
...
# TrafficSignal class (same as original)
class TrafficSignal:
...
# Initialization of signals with fixed times
def initialize():
    for i in range(noOfSignals):
        if i == 0:
            signals.append(TrafficSignal(0, TRADITIONAL_YELLOW, TRADITIONAL_GREEN))
        else:
            signals.append(TrafficSignal(TRADITIONAL_RED, TRADITIONAL_YELLOW, TRADITIONAL_GREEN))
    repeat()
...
def repeat():
    ...
    while(signals[currentGreen].yellow>0):
        ...
        time.sleep(1)
    currentYellow = 0
    # Reset all signal times to fixed values
    signals[currentGreen].green = TRADITIONAL_GREEN
    signals[currentGreen].yellow = TRADITIONAL_YELLOW
    signals[currentGreen].red = TRADITIONAL_RED
...
def run_traditional_simulation():
    ...
    pygame.display.set_caption("TRADITIONAL SIMULATION")
...
# Ensure all functions are defined before calling the main entry point
if __name__ == "__main__":
    run_traditional_simulation()

# ...existing code for vehicle generation, simulationTime, and Main class (copy from simulation.py, but remove setTime and emergency logic)...

# You must copy the rest of the simulation logic (vehicle, images, Main, etc.) from simulation.py and remove any AI/adaptive/ambulance logic for a pure traditional timer.
# This is a template. Let me know if you want the full working file generated for you!
